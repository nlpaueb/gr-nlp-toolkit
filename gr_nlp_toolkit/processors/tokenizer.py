...
def create_mask_and_tokens(input_tokens: List[str], input_ids: List[int]) -> Tuple[List[str], List[Token]]:
    mask = []
    tokens = []

    # for each token
    for t, i in zip(input_tokens, input_ids):
        # it isn't a sub-word
        if not t.startswith("##"):
...
class Tokenizer(AbstractProcessor):
    ...
    def __call__(self, doc: Document) -> Document:
        ...
        doc.mask, doc.tokens = create_mask_and_tokens(convert_to_tokens(doc.input_ids), remove_special_tokens(doc.input_ids))
